...
export function Discovery({initialCandidates,jobs:initialJobs,discoveryFailed,onFirmApproved,notify}:Props){
  ...
  const [approvalWarningId,setApprovalWarningId]=useState<string|null>(null);
  ...
  const backfillResearch=async()=>{if(backfillPending)return;setBackfillPending(true);try{const response=await fetch("/api/discovery/web-research/backfill",{method:"POST"});const body=await response.json();if(!response.ok)throw new Error(body.error);notify(`${body.queued} candidate${body.queued===1?"":"s"} queued for web research`);await refreshCandidatesAndJobs();}catch(error){logSafeError("backfill web research",error);notify("Candidates could not be queued for web research.");}finally{setBackfillPending(false);}};
  ...
  return <section className="page discovery-page"><div className="page-title compact"><div><span className="eyebrow">PROSPECT OPERATIONS</span><h1>Discovery</h1><p>Review real prospects before they enter your partner database.</p></div><button className="primary" onClick={()=>setTab("Discovery jobs")}><Play size={16}/>New discovery job</button></div>
    ...
    {tab==="Review queue"?<><div className="review-toolbar"><div className="review-search"><Search size={15}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search candidates..."/></div><label className="select"><select value={status} onChange={e=>setStatus(e.target.value)}>{["All statuses","New","Approved","Rejected","Duplicate","Needs Review"].map(x=><option key={x}>{x}</option>)}</select><ChevronDown size={14}/></label><span>{visible.length} results</span><button disabled={backfillPending} onClick={backfillResearch}><FileSearch size={14}/>{backfillPending?"Queueing…":"Backfill web research"}</button></div>
      {selected.length>0&&bulkResearchPending>0&&<p className="bulk-research-warning"><AlertTriangle size={13}/>{bulkResearchPending} selected candidate{bulkResearchPending===1?" has":"s have"} unfinished web research. Approval copies only findings already accepted.</p>}
