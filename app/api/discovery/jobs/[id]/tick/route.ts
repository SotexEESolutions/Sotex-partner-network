...
type QueryRow={id:string;category:string;query_text:string;page_number:number;next_page_token:string|null;attempt_count:number;places_returned:number;candidates_staged:number;duplicates_skipped:number;firm_matches_flagged:number};
...
export async function POST(_request:Request,context:{params:Promise<{id:string}>}){
  ...
  try{
    const result=await searchGooglePlaces({query:nextQuery.query_text,pageToken:nextQuery.next_page_token,pageSize:20});
    const places=(result.places??[]).filter((place)=>place.id&&place.displayName?.text);
    const rows=places.map((place)=>mapGooglePlace(place,{jobId:job.id,market:job.market,region:job.region,category:nextQuery.category}));
    ...
    const staged=insertion.data?.length??0,duplicates=places.length-staged,flagged=(insertion.data??[]).filter((candidate)=>candidate.duplicate_status!=="No Match").length;
