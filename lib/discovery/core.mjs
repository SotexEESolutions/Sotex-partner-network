...
export function isMailMergeEligible(firm){const primary=firm.contacts.find(c=>c.primary);return ["A+","A","B"].includes(firm.grade)&&Boolean(primary?.email)&&primary.emailStatus!=="Invalid"&&!firm.outreach.some(o=>["Partner","Not Interested"].includes(o.status));}

export const DISCOVERY_DAILY_REQUEST_LIMIT_DEFAULT=60;
export function parseDiscoveryDailyLimit(value,fallback=DISCOVERY_DAILY_REQUEST_LIMIT_DEFAULT){
